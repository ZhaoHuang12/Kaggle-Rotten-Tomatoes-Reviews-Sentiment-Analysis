...
word_id_train_val = np.load("word_id_train.npy")
sequence_len_train_val = np.load("sequence_len_train.npy")
sentiment_vectors_train_val = np.load("sentiment_vectors_train.npy")

word_id_test = np.load("word_id_test.npy")
sequence_len_test = np.load("sequence_len_test.npy")

seq_len = len(word_id_train_val[0])
VOCAB_LENGTH,EMBEDDING_DIMENSION = word_embeddings.shape
#finish processing training data and test data, start to construct the neural network
...
#embedding layer, convert word ids to word vectors
...
with tf.Session() as sess:
  ...
  data = pd.read_csv('all/test.tsv', sep='\t')
  data['Sentiment'] = predictions
  data.drop(['Phrase', 'SentenceId'], axis=1, inplace=True)
  print(data.head(10))
  data.to_csv('all/Submission.csv', header=True, index=None, sep=',')
